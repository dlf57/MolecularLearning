...
creating a Typed Bond Angle Torsion representations.
'''
...
def batty(mol_file, bags, bag_sizes):
    '''
    ...
    for torsion in ob.OBMolTorsionIter(mol.OBMol):
        ...
        if (aType < dType):
            ...
            torsion_angle = radians(abs(mol.OBMol.GetTorsion(a, b, c, d)))
            bag_set[torsion_type].append(np.float16(torsion_angle))
        else:
    ...
    for i in range(len(bag_keys)):
        ...
        batty.append(bag_set[bag_keys[i]])

    # flatten bty into one list and store as a np.array
    batty = np.array(list(chain.from_iterable(batty)))
